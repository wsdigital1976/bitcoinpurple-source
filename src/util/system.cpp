...
const char * const BITCOINPURPLE_CONF_FILENAME = "bitcoinpurple.conf";
const char * const BITCOINPURPLE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinpurple-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITCOINPURPLE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinPurple
    // macOS: ~/Library/Application Support/BitcoinPurple
    // Unix-like: ~/.bitcoinpurple
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinPurple";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinPurple";
#else
    // Unix-like
    return pathRet / ".bitcoinpurple";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BITCOINPURPLE_CONF_FILENAME));
}
