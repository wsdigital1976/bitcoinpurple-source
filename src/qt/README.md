This directory contains the source code for the BitcoinPurple Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### bitcoinpurplegui.(h/cpp)

- Represents the main window of the BitcoinPurple UI.
...
- ClientModel is used by the main application `bitcoinpurplegui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bitcoinpurple:... link from a browser).
...
* UI elements like BitcoinPurpleAmountField, which inherit from QWidget.
* `bitcoinpurplestrings.cpp`: automatically generated
* `bitcoinpurpleunits.(h/cpp)`: BTCP / mBTCP / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bitcoinpurple_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bitcoinpurple-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitcoinpurple-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bitcoinpurple-qt`
9. You're all set! Start developing, building, and debugging the BitcoinPurple Core GUI
