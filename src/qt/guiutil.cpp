...
#include <qt/bitcoinpurpleaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinPurple address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitcoinPurpleAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinPurpleAddressCheckValidator(parent));
}
...
bool parseBitcoinPurpleURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitcoinpurple: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoinpurple"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinPurpleUnits::parse(BitcoinPurpleUnit::BTCP, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinPurpleURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoinPurpleURI(uriInstance, out);
}

QString formatBitcoinPurpleURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitcoinpurple:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinPurpleUnits::format(BitcoinPurpleUnit::BTCP, info.amount, false, BitcoinPurpleUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinPurpleConf()
{
    ...
    /* Open bitcoinpurple.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinPurple.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinPurple (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BitcoinPurple (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinPurple*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcoinpurple.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitcoinpurple-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitcoinpurple.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinPurple\n";
        else
            optionFile << strprintf("Name=BitcoinPurple (%s)\n", chain);
