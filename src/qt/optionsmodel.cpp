...
/** Call node.updateRwSetting() with BitcoinPurple 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitcoinPurple 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitcoinpurple/bitcoinpurple/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitcoinpurple 22.x will not try to read these.
...
//! Convert enabled/size values to bitcoinpurple -prune setting.
...
//! Get pruning enabled value to show in GUI from bitcoinpurple -prune setting.
...
//! Get pruning size value to show in GUI from bitcoinpurple -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoinPurpleUnit")) {
        settings.setValue("DisplayBitcoinPurpleUnit", QVariant::fromValue(BitcoinPurpleUnit::BTCP));
    }
    QVariant unit = settings.value("DisplayBitcoinPurpleUnit");
    if (unit.canConvert<BitcoinPurpleUnit>()) {
        m_display_bitcoinpurple_unit = unit.value<BitcoinPurpleUnit>();
    } else {
        m_display_bitcoinpurple_unit = BitcoinPurpleUnit::BTCP;
        settings.setValue("DisplayBitcoinPurpleUnit", QVariant::fromValue(m_display_bitcoinpurple_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitcoinpurple.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitcoinpurple_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinPurpleUnit>() == m_display_bitcoinpurple_unit) return;
    m_display_bitcoinpurple_unit = new_unit.value<BitcoinPurpleUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinPurpleUnit", QVariant::fromValue(m_display_bitcoinpurple_unit));
    Q_EMIT displayUnitChanged(m_display_bitcoinpurple_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitcoinpurple/bitcoinpurple/pull/8273
    ...
    // (https://github.com/bitcoinpurple-core/gui/issues/567).
