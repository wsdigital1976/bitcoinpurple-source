...
#include <config/bitcoinpurple-config.h>
#endif

#include <qt/bitcoinpurple.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitcoinPurpleUnit>("BitcoinPurpleUnit");
#else
    qRegisterMetaType<BitcoinPurpleUnit>("BitcoinPurpleUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoinpurple_de.qm (shortcut "de" needs to be defined in bitcoinpurple.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoinpurple_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoinpurple.qrc)
...
static const char* qt_argv = "bitcoinpurple-qt";

BitcoinPurpleApplication::BitcoinPurpleApplication()
...
void BitcoinPurpleApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinPurpleApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinPurpleGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinPurpleApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinPurpleApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinPurpleApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinPurpleGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinPurpleGUI::quitRequested, this, &BitcoinPurpleApplication::requestShutdown);
...
void BitcoinPurpleApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinPurpleApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinPurpleApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinPurpleApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinPurpleApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinPurpleApplication::handleRunawayException);
    connect(this, &BitcoinPurpleApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinPurpleApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinPurpleApplication::parameterSetup()
{
...
void BitcoinPurpleApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinPurpleApplication::requestInitialize()
{
...
void BitcoinPurpleApplication::requestShutdown()
{
...
void BitcoinPurpleApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bitcoinpurple: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitcoinPurpleGUI::handlePaymentRequest);
            connect(window, &BitcoinPurpleGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitcoinPurpleApplication::handleRunawayException(const QString &message)
{
...
void BitcoinPurpleApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinPurpleApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinPurpleApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinPurpleGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoinpurple);
    Q_INIT_RESOURCE(bitcoinpurple_locale);
    ...
    /// 6-7. Parse bitcoinpurple.conf, determine network, switch to network specific
    ...
    // bitcoinpurple: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
