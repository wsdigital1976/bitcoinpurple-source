...
static void SetupBitcoinPurpleTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinPurpleTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinpurple-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoinpurple-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinpurple transaction\n"
                "or:     bitcoinpurple-tx [options] -create [commands]   Create hex-encoded bitcoinpurple transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinpurple transaction
