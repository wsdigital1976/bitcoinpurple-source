...
Below are some notes on how to build BitcoinPurple Core for Windows.

The options known to work for building BitcoinPurple Core on Windows are:
...
Note that for WSL the BitcoinPurple Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinpurple, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinpurple`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinpurple
