...
The BitcoinPurple Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinpurple wire protocol and relay, making
...
The ZeroMQ feature in BitcoinPurple Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinpurpled:
...
For instance:
    ...
    $ bitcoinpurpled -zmqpubhashtx=tcp://127.0.0.1:213495 \
               -zmqpubhashtx=tcp://192.168.1.2:213495 \
               -zmqpubhashblock="tcp://[::1]:213496" \
               -zmqpubrawtx=ipc:///tmp/bitcoinpurpled.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in bitcoinpurple.conf.
...
From the perspective of bitcoinpurpled, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinpurpled directly. Furthermore, no information is
...
using. BitcoinPurpled appends an up-counting sequence number to each
