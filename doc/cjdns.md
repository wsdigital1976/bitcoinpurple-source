# CJDNS support in BitcoinPurple Core

It is possible to run BitcoinPurple Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinPurple Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinPurple network and individual nodes.
...
before it will work with your BitcoinPurple Core node. This is described in steps
...
## Run BitcoinPurple Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinPurple Core
...
When enabled, this option tells BitcoinPurple Core that it is running in an
...
IPv6 local network. This helps BitcoinPurple Core perform better address management:
...
CJDNS support was added to BitcoinPurple Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinpurple-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitcoinpurple/bitcoinpurple/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinPurple Core

There are several ways to see your CJDNS address in BitcoinPurple Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinpurple-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoinpurple-cli getpeerinfo`).
