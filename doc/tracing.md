# User-space, Statically Defined Tracing (USDT) for BitcoinPurple Core

BitcoinPurple Core includes statically defined tracepoints to allow for more
...
**Note**: BitcoinPurple Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitcoinPurple Core
...
Multiple tools can list the available tracepoints in a `bitcoinpurpled` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitcoinPurple Core, use `info probes` in `gdb`:

```
$ gdb ./src/bitcoinpurpled
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/bitcoinpurpled
stap net        outbound_message 0x0000000000107c05 /src/bitcoinpurpled
stap validation block_connected  0x00000000002fb10c /src/bitcoinpurpled
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitcoinPurple Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinpurpled | grep NT_STAPSDT -A 4 -B 2
