# PSBT Howto for BitcoinPurple Core

Since BitcoinPurple Core 0.17, an RPC interface exists for Partially Signed BitcoinPurple
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinpurple/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinPurple transactions that are not fully signed
...
#### Multisig with multiple BitcoinPurple Core instances
...
BitcoinPurple Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinpurple-cli` is used.
...
Later, when *V* BTCP has been received on *Amulti*, and Bob and Carol want to
