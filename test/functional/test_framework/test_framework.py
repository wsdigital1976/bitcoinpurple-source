...
TMPDIR_PREFIX = "bitcoinpurple_func_test_"
...
class BitcoinPurpleTestMetaClass(type):
    """Metaclass for BitcoinPurpleTestFramework.

    Ensures that any attempt to register a subclass of `BitcoinPurpleTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitcoinPurpleTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitcoinPurpleTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitcoinPurpleTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitcoinPurpleTestFramework(metaclass=BitcoinPurpleTestMetaClass):
    """Base class for a bitcoinpurple test script.

    Individual bitcoinpurple test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitcoinpurpleds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitcoinpurpleds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitcoinpurple-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitcoinpurpled = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinpurpled" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinpurplecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinpurple-cli" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinpurpleutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinpurple-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoinpurpled = os.getenv("BITCOINPURPLED", default=fname_bitcoinpurpled)
        self.options.bitcoinpurplecli = os.getenv("BITCOINPURPLECLI", default=fname_bitcoinpurplecli)
        self.options.bitcoinpurpleutil = os.getenv("BITCOINPURPLEUTIL", default=fname_bitcoinpurpleutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcoinpurpleds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BitcoinPurpleRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoinpurpled', self.options.bitcoinpurpled) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bitcoinpurple-cli', self.options.bitcoinpurplecli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                bitcoinpurpled=binary[i],
                bitcoinpurple_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple bitcoinpurpleds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitcoinpurpled test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitcoinpurpled test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoinpurpled's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitcoinpurpled=self.options.bitcoinpurpled,
                    bitcoinpurple_cli=self.options.bitcoinpurplecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoinpurple.conf
    ...
    def skip_if_no_bitcoinpurpled_tracepoints(self):
        """Skip the running test if bitcoinpurpled has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoinpurpled has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoinpurpled_zmq(self):
        """Skip the running test if bitcoinpurpled has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoinpurpled has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitcoinpurple-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitcoinpurple-wallet has not been compiled")

    def skip_if_no_bitcoinpurple_util(self):
        """Skip the running test if bitcoinpurple-util has not been compiled."""
        if not self.is_bitcoinpurple_util_compiled():
            raise SkipTest("bitcoinpurple-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoinpurple-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitcoinpurple-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bitcoinpurple-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitcoinpurple-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoinpurple_util_compiled(self):
        """Checks whether bitcoinpurple-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOINPURPLE_UTIL")
