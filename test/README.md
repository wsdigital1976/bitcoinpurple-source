This directory contains integration tests that test bitcoinpurpled and its
...
bitcoinpurpled and bitcoinpurple-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bitcoinpurple-util, bitcoinpurple-tx, ...).
...
Before tests can be run locally, BitcoinPurple Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the bitcoinpurpled nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitcoinpurpled
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitcoinpurpled nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitcoinpurpled processes are running.

On linux, the test framework will warn if there is another
bitcoinpurpled process running when the tests are started.

If there are zombie bitcoinpurpled processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitcoinpurpled processes running on the system, so should not be used if any non-test
bitcoinpurpled processes are being run.**

```bash
killall bitcoinpurpled
```

or

```bash
pkill -9 bitcoinpurpled
```
...
sure bitcoinpurpled processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitcoinpurpled `debug.log`s will all be dumped
...
`test_framework.log` and bitcoinpurpled `debug.log`s can be combined into a single
...
call methods that interact with the bitcoinpurpled nodes-under-test.

If further introspection of the bitcoinpurpled instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitcoinpurpled.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitcoinpurpled <pid>
```
