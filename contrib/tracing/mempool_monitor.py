...
""" Example logging BitcoinPurple Core mempool events using the mempool:added,
...
def main(bitcoinpurpled_path):
    bitcoinpurpled_with_usdts = USDT(path=str(bitcoinpurpled_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitcoinpurpled_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitcoinpurpled_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitcoinpurpled_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitcoinpurpled_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitcoinpurpled_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bitcoinpurpled")
        exit(1)
