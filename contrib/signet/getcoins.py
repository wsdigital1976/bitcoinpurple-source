...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinpurple-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinpurple-cli', help='bitcoinpurple-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinPurple address to which the faucet should send')
...
parser.add_argument('bitcoinpurple_cli_args', nargs='*', help='Arguments to pass on to bitcoinpurple-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinpurple_cli_args == []:
    args.bitcoinpurple_cli_args = ['-signet']


def bitcoinpurple_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinpurple_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinpurple_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinpurple_cli(['getnewaddress', 'faucet', 'bech32'])
