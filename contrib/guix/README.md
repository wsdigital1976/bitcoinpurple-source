# Bootstrappable BitcoinPurple Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinPurple
Core builds.
...
To build BitcoinPurple Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinPurple Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinpurple-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoinpurple-core/guix.sigs).
