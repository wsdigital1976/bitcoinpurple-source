...
def parse_config_into_btcp_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"BitcoinPurple Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinpurple_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinpurple_config.h'), "w", encoding="utf8") as btcp_config:
        btcp_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='BitcoinPurple-core msbuild configuration initialiser.')
    ...
    parse_config_into_btcp_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bitcoinpurple_config.h'), os.path.join(SOURCE_DIR, 'config/bitcoinpurple-config.h'))
